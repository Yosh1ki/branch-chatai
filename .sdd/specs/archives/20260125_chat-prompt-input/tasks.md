...
- [ ] 4.3 要件の受入基準を満たすことを確認する
  ...
  - PII非保存の確認
  - 実行済み: `npm test` (pass)
  - コード確認: 親チェーン履歴の取得/40件超過時の要約/429応答/タイトル生成1回は実装済み
  - 未検証: 実データでの分岐履歴範囲、外部モデレーション連携、モデル呼び出し/再試行/フォールバック、PII非保存の運用確認
  - 要確認: `src/lib/model-invoker.ts` にフォールバック1段の実装が見当たらないため、仕様との差分有無を確認する
  - 次のアクション: APIキー/DB環境を用意して `/api/chat` の実行確認を行い、受入基準を実地検証する
