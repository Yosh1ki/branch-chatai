...
- Payment requirements: Stripe integration must capture `stripe_customer_id` and `stripe_subscription_id` for Pro accounts, expose checkout/portal entry points from the settings modal, and synchronize plan state via Stripe webhooks.
